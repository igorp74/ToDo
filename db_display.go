...
func ListTasks(tm *TodoManager, projectFilter, contextFilter, tagFilter, statusFilter, startBefore, startAfter, dueBefore, dueAfter, sortBy, order string, format int) {
	query := `
		SELECT
	...
	for rows.Next() {
		...
		case DisplayFull:
			...
			if len(task.ProjectName.String) > 0 {
				titleParts = append(titleParts, "Project: " + fg_green + task.ProjectName.String + style_reset)
			}
			if len(task.Tags) > 0 {
				titleParts = append(titleParts, "Tags: " + fg_blue + strings.Join(task.Tags, ", ") + style_reset)
			}
			if len(task.Contexts) > 0 {
				titleParts = append(titleParts, "Context: " + fg_magenta + strings.Join(task.Contexts, ", ") + style_reset)
			}
			...
			titleParts = append(titleParts, "Status: " + status_str)
