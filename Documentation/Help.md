...
### Delete
  `del`   Delete a task by ID, or filtered by project, tag, context, or status
  
    --ids           Comma-separated IDs or ID ranges of tasks to delete (e.g., '1,2,3-5,10'). If filters are used, applies to the filtered set.
    ...
    --confirm       Confirm deletion without prompt when using filters
...
### Holidays
  `holiday`       Manage holidays.
    ...
    Subcommands for holiday:
      ...
      holiday update    Update an existing holiday.
          -i, --id      ID of the holiday to update (required)
          -d, --date    New date of the holiday (YYYY-MM-DD). Use empty string with flag to clear.
          -n, --name    New name of the holiday. Use empty string with flag to clear.
