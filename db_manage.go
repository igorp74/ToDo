...
func (tm *TodoManager) GetHolidays() (map[string]Holiday, error) { // Use map[YYYY-MM-DD]Holiday for easy lookup
	...
	for rows.Next() {
		...
		// Holidays are stored as "YYYY-MM-DD" strings in the DB.
		// When reconstructing the Holiday struct, we'll parse it back, location doesn't strictly matter here as it's date-only.
