...
func main() {
	...
	// Update Note command - MODIFIED TO USE TASK ID AND DISPLAY INDEX
	updateNoteCmd := parser.NewCommand("update-note", "Update an existing note by its display index for a given task.")
	updateNoteTaskID := updateNoteCmd.Int("task-id", "i", &Options{Required: true, Help: "ID of the task the note belongs to"})
	updateNoteDisplayIndex := updateNoteCmd.Int("note-index", "n", &Options{Required: true, Help: "Display index of the note within the task (1-based, as shown in 'list' command)"})
	...
	case updateNoteCmd.Parsed:
		...
		// Resolve the display index to the actual database note ID
		dbNoteID, err := tm.GetNoteIDByTaskAndIndex(int64(*updateNoteTaskID), *updateNoteDisplayIndex)
		if err != nil {
			log.Fatalf("Error resolving note index: %v", err)
		}
		tm.UpdateNote(dbNoteID, *updateNoteDescription, *updateNoteTimestamp, updateNoteCmd.GetFlag("timestamp").IsSet)
